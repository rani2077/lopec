...
### 수정됨 (Fixed)
* 시뮬레이터 진입시 악세서리 초기화 버그 수정
* 시뮬레이터 진입시 악세서리 초기화 버그 수정2
* 시뮬레이터 광휘의 보석 적용
* 시뮬레이터 깨달음&도약 카르마 레벨 조정 버튼 추가
* 시뮬레이터 깨달음&도약 카르마 레벨에 따른 점수 적용
* 시뮬레이터 발키리 직업 진입 오류 수정
* 시뮬레이터 도약 랭크 선택 오류 수정
* 메인 페이지 어빌리티스톤 티어 표기 오류 수정
* 메인 페이지 에스더 무기 티어 표기 오류 수정

### 변경됨 (Changed)
* 메인 페이지 도약 레벨에 따른 점수 적용
* 인게임 전투력 아이콘 추가 및 수정
* 시뮬레이터 서폿 깨달음 노드에 따른 낙인력 반영
* 서폿 직업군 팔찌 효율 계산식 수정
* 서폿 직업군 깨달음 레벨에 따른 버프력 밸류 수정
...
### 수정됨 (Fixed)
* 시뮬레이터에서 서포터 직업군의 **광휘 보석**이 점수에 포함되지 않던 오류 수정
* **광휘의 보석** 착용 시 에러 발생 이슈 해결

### 변경됨 (Changed)
* **로스트아크 대규모 패치 대응 작업 - 2**
* 서포터 **깨달음 사이드 노드 개편**에 따른 **낙인력 필터** 작성
* **로스트아크 대규모 패치 대응 작업 - 1**
* "**각성**" 각인 개편에 따른 **밸류 개편** 적용
* 서포터 **점수 계산식 소폭 수정**
...
### 변경됨 (Changed)
* **로스트아크 API 개편 대응 작업**
* **팔찌 툴팁 참조** element number 변경 및 **정규식** 수정
* **액세서리 참조** element number 변경 및 **정규식** 수정
* **팔찌 옵션 툴팁 HTML 구조 변경**에 따른 대응 작업
* **카르마** 기존 DB 참조 로직 제거, **API 직접 참조**로 변경
...
### 수정됨 (Fixed)
* **보석 오류** 수정
* **진화 카르마**가 없을 때 생기는 오류 해결
* 서포터 **팔찌 딜증 오류** 수정 (딜러 직업군이 서포터 이중 옵션 착용 시 점수 오류 수정)
* 시뮬레이터에서 딜 세팅 서포터의 상세 정보가 서포터로 뜨던 오류 수정
* 모바일 페이지 **상세 정보 아이콘** 정상 적용

### 추가됨 (Added)
* **진화 카르마 랭크**를 통한 점수 계산식 작성 및 추가 (딜/서포터 역할에 맞추어 랭크에 따른 스펙업 적용, 시뮬레이터에 진화 카르마 랭크/레벨 조율 로직 및 UI 추가, 모바일 페이지에도 동일 작업 반영)
* **진화 카르마 점수 도입 준비** (딜러 진화형 피해 수치 추가)
...
### 추가됨 (Added)
* **딜러 고도화 준비 - 1** (크리티컬 확률 및 대미지 정리)


## [2025-05-16]

### 수정됨 (Fixed)
* **보석 딜 지분** 일부 시뮬레이터 미적용 수정

### 변경됨 (Changed)
* **5월 2주차 중앙값 업데이트**
* **두동 호크 블레이드 스톰 딜 지분** 추가


## [2025-05-15]

### 수정됨 (Fixed)
* **보석 딜 지분 데이터 없음 오적용** 오류 개선
...
### 변경됨 (Changed)
* **공격력 계산식 정리** (최종 공격력 계산식 정리, 팔찌 효율 계산식 정리)
...
### 추가됨 (Added)
* **포식자 딜 지분 데이터 추가** (마운틴 채용 포식자 딜 지분 데이터 추가)
...
### 수정됨 (Fixed)
* 아바타 없을 시 시뮬레이터 **진입 안 되는 오류** 수정

### 추가됨 (Added)
* **공지 작성**

### 변경됨 (Changed)
* **핫픽스** (비기 & 사시 밸런스 패치에 따른 보석 전용 직업 "데이터 없음" 처리)
* **핫픽스** (도약 포인트 밸류 수정, 팔찌 이슈 수정)
* **추가 수정** (서포터 팔찌 계산식 수정, 에테르 포식자 효율 수정)


## [2025-05-06]

### 수정됨 (Fixed)
* 시뮬레이터 **DB 미적용** 해결 (DB 특성합 데이터가 시뮬레이터에 덮어씌워지도록 수정)
* 시뮬레이터 **에테르 포식자 오효율 이슈** 수정
* **후원 배너 링크** 연결 문제 수정
* 시뮬레이터 **팔찌 특화/신속에 DB값**이 적용되지 않던 문제 해결

### 변경됨 (Changed)
* **후원 배너 링크 연결**
* **악세 효율 계산식** 수정
* **팔찌 효율 계산식** 수정
* **로펙딸깍 악추피 적용**, 개발용 스케일 해제
* **공지 및 광휘 오류** 수정
